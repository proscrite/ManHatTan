...
Instance = namedtuple('Instance', 'p t fv h a lang right wrong ts uid lexeme index'.split())
...
def read_data(input_file, method, omit_bias=False, omit_lexemes=False, max_lines=None):
    ...
    for i, row in enumerate(reader):
        ...
        right = float(row['history_correct'])
        wrong = seen - right
        #right_this = int(row['session_correct'])   # Accept negative values too
        right_this = float(row['session_correct'])
        ...
        if method == 'leitner':
            ...
            #print('In HLR script, wrong = ', wrong)
        ...
        instances.append(Instance(p, t, fv, h, (right+2.)/(seen+4.), lang, right_this, wrong_this, timestamp, user_id, lexeme_string, i))
        if i % 1000000 == 0:
