...
<ScrollCell>:
    background_color: [root.red_val, 0., abs(root.red_val-0.73), 1] if root.is_even else [root.red_val, 0., abs(root.red_val-0.83), 1]
    ...
    canvas.before:
        ...
        Rectangle:
            ...
            size: self.size
    text: root.text
    disabled: True if root.text == '' else False
    font_size: "12dp"
    halign: "center"
    valign: "middle"
    text_size: self.size
...
class ScrollCell(Button):
    ...
    def on_release(self):
        ...
        new_red = True
        print(self.coords)
        App.get_running_app().modify_pdf(self.coords)
        self.parent.parent.parent.parent._generate_table()
        self.parent.parent.update_from_scroll()
...
class TableData(RecycleView):
    ...
    def __init__(self, list_dicts=[], color_map=[], *args, **kwargs):
        ...
        for i, (ord_dict, col_dict) in enumerate(zip(list_dicts, color_map)):
            ...
            for j, (text, red_val) in enumerate(zip(row_vals, row_cols)):
                if text == 'None':
                    text = ''

                self.data.append({'text': text, 'is_even': is_even, 'red_val':red_val, 'coords': [i,j]})
...
class ContextEditor(App):
    ...
    def build(self):
        ...
        opGrid.add_widget(backButton)
        mainGrid.add_widget(opGrid)

        return mainGrid

    def modify_pdf(self, coords: []):
        self.pdf.iloc[coords[0], :] = 0. # First set whole row to 0 (yes, discard all the work of predicting most uncommon word in a brush >:)
        self.pdf.iloc[coords[0], coords[1]] = 1.


    def saveNexit(self, instance):
        self.lipstick.to_csv('~/Documents/ManHatTan/LIPSTICK/Io_Uccido_edited.ctx', index=False)
        App.stop(self)
